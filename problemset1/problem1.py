...
# left, right that ddedscribes/describes the search region as shown below
import logging
...
def findCrossoverIndexHelper(x, y, left, right):
    ...
    logging.info(f"findCrossoverIndexHelper({x},{y},left={left},right={right})")
    assert(len(x) == len(y))
    assert(left >= 0)
    assert(left <= right-1)
    assert(right < len(x))
    ...
    logging.info(f"x[mid]={x[mid]}, y[mid]={y[mid]}")
    logging.info(f"x[mid+1]={x[mid+1]}, y[mid+1]={y[mid+1]}")
