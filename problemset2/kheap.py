...
class TopKHeap:
    ...
    def satisfies_assertions(self):
        ...
        for i in range(len(self.A) - 1):
            assert (
                self.A[i] <= self.A[i + 1]
            ), f"Array A fails to be sorted at position {i}, {self.A[i], self.A[i+1]}"
        ...
        for i in range(len(self.A)):
            assert (
                self.A[i] <= self.H.min_element()
            ), f"Array element A[{i}] = {self.A[i]} is larger than min heap element {self.H.min_element()}"
    ...
    def insert_into_A(self, elt):
        ...
        if len(self.A) > self.k:
            displaced_elt = self.A[-1]
            del self.A[-1]
        return displaced_elt
    ...
    def insert(self, elt):
        ...
        # your code here
            ...
            heap_ele = self.insert_into_A(elt)
            self.H.insert(heap_ele)
    ...
    def delete_top_k(self, j):
        ...
        assert (
            self.size() > k
        )  # we need not handle the case when size is less than or equal to k
        assert j >= 0
        assert j < self.k
        # your code here
        del self.A[j]
        array_ele = self.H.min_element()
        self.H.delete_min()
        self.A.append(array_ele)
